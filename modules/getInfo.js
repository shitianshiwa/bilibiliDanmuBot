...
const getAllVideos = async (pageSize) =>{  //优化:能否省略第一次的无用请求？复用第一次的请求结果?
    const {body} = await got.get(`https://api.bilibili.com/x/space/arc/search?mid=${config.get('streamInfo.owner')}&pn=1&ps=1&jsonp=jsonp`,{
        responseType: "json"
    })
    if (body.code !== 0){
...
const getVideoByOptions = async (pageNumber,pageSize) =>{
    const {body} = await got.get(`https://api.bilibili.com/x/space/arc/search?mid=${config.get('streamInfo.owner')}&pn=${pageNumber}&ps=${pageSize}&jsonp=jsonp`,{
        responseType: "json"
    })
